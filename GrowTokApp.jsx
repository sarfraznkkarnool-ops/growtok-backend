...
import { Heart, MessageCircle, Share2, Plus, Home, Compass, User, Search, X, Sprout, Music2, LogOut, ArrowLeft, Volume2, VolumeX } from "lucide-react";
...
// ---------------------------------------------------------------------------
// AdSense — fill these in once your AdSense application is approved for the
// real domain this app is deployed on. Until both are set, the feed falls
// back to the mock ad slide so the demo still shows what the slot looks like.
// ---------------------------------------------------------------------------
const ADSENSE_CLIENT_ID = "ca-pub-2598607795840843"
const ADSENSE_SLOT_ID = "2569192662"
...
// ---------------------------------------------------------------------------
// API layer — with automatic access-token refresh.
// A 30-minute access token going stale mid-session was the cause of "like
// happens then reverts" and "invalid or expired token" on upload: any call
// after the token expired failed, and the UI rolled back its optimistic
// update. Now a single 401 triggers one silent refresh + retry before
// giving up, using the refresh token issued at login.
// ---------------------------------------------------------------------------
    ...
    if (res.status === 401 && auth && current?.refresh_token) {
      try {
        ...
        if (refreshRes.ok) {
          ...
          setTokens(null); // refresh token itself is dead — back to login
        }
      } catch (_) {
        setTokens(null);
      }
...
// ---------------------------------------------------------------------------
// Connect / Auth screen — API base is fixed now, so there's one less thing
// to get wrong before signing up.
// ---------------------------------------------------------------------------
function ConnectScreen({ onAuthed }) {
  ...
  const inputStyle = {
    width: "100%", boxSizing: "border-box", background: T.bg, border: `1px solid ${T.line}`,
    borderRadius: 12, padding: "11px 14px", color: T.paper, fontFamily: FONT, fontSize: 14, outline: "none", marginBottom: 10,
  };
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 26px", boxSizing: "border-box" }}>
      ...
      <h2 style={{ color: T.paper, fontFamily: FONT, fontSize: 20, fontWeight: 700, margin: "0 0 4px" }}>
        {mode === "login" ? "Log in to GrowTok" : "Create an account"}
      </h2>
      <p style={{ color: T.paperDim, fontFamily: FONT, fontSize: 12.5, margin: "0 0 18px" }}>
        {mode === "login" ? "Welcome back" : "Join GrowTok"}
      </p>
      ...
      <button
        ...
        style={{ background: T.lime, color: T.bg, border: "none", borderRadius: 12, padding: "12px 0", fontFamily: FONT, fontWeight: 700, fontSize: 14.5, cursor: "pointer", opacity: busy ? 0.7 : 1, marginBottom: 12 }}
      >
...
// ---------------------------------------------------------------------------
// Ad Slide (mock) + AdSense in-feed slide
// ---------------------------------------------------------------------------
function AdSlide({ ad, onSkip }) {
  ...
  return (
    <div style={{ position: "relative", height: "100%", width: "100%", scrollSnapAlign: "start", flexShrink: 0, background: ad.gradient, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "0 32px", boxSizing: "border-box" }}>
...
function AdSenseSlide() {
  ...
  return (
    <div style={{ height: "100%", width: "100%", scrollSnapAlign: "start", flexShrink: 0, background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 16px", boxSizing: "border-box", position: "relative" }}>
      <span style={{ position: "absolute", top: 16, left: 16, color: T.paperDim, fontSize: 11, letterSpacing: 1, fontFamily: FONT, border: `1px solid ${T.line}`, borderRadius: 4, padding: "3px 7px" }}>SPONSORED</span>
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 10, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      ...
      <div style={{ position: "relative", background: T.card, borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: "62%", display: "flex", flexDirection: "column", borderTop: `1px solid ${T.line}` }}>
        ...
        <div style={{ overflowY: "auto", padding: "12px 18px", display: "flex", flexDirection: "column", gap: 16, minHeight: 80 }}>
          ...
          {comments.map((c) => (
            <div key={c.id} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
              <div style={{ width: 30, height: 30, borderRadius: "50%", background: T.limeDim, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: T.bg, fontFamily: FONT, fontWeight: 700, fontSize: 12 }}>
        ...
        <div style={{ padding: 14, borderTop: `1px solid ${T.line}`, display: "flex", gap: 8 }}>
          ...
          <button onClick={submit} disabled={posting || !text.trim()} style={{ background: T.lime, color: T.bg, border: "none", borderRadius: 18, padding: "0 16px", fontFamily: FONT, fontWeight: 700, fontSize: 13, cursor: "pointer", opacity: posting || !text.trim() ? 0.6 : 1 }}>Post</button>
        </div>
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// Feed slide — real playback, tap or the speaker icon toggles sound.
// Videos start muted (browsers block autoplay-with-sound); tapping unmutes.
// ---------------------------------------------------------------------------
function FeedSlide({ video, liked, likeBusy, isFollowing, followBusy, showFollow, onLike, onComment, onFollow, onShare, onOpenProfile }) {
  ...
  return (
    <div style={{ position: "relative", height: "100%", width: "100%", scrollSnapAlign: "start", flexShrink: 0, background: "#000", overflow: "hidden" }}>
      {src ? (
        <video ref={videoRef} src={src} loop muted={isMuted} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover", cursor: "pointer" }} onClick={toggleSound} />
      ) : (
        <div style={{ width: "100%", height: "100%", background: "linear-gradient(160deg,#2d3b1f,#141310 70%)" }} />
      )}

      {src && (
        <button onClick={toggleSound} aria-label={isMuted ? "Unmute" : "Mute"} style={{ position: "absolute", top: 18, right: 18, zIndex: 4, background: "rgba(0,0,0,0.55)", border: `1px solid ${T.line}`, borderRadius: "50%", width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
      ...
      <div style={{ position: "absolute", left: 16, right: 84, bottom: 28, zIndex: 3 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <button onClick={() => onOpenProfile(video.owner)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 36, height: 36, borderRadius: "50%", background: T.lime, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: T.bg, fontFamily: FONT, fontSize: 14 }}>
...
// ---------------------------------------------------------------------------
// Creator profile — opened by tapping a username on any video. Shows the
// creator's own videos (filtered from what's already loaded) so no extra
// backend endpoint is needed.
// ---------------------------------------------------------------------------
function CreatorProfileScreen({ owner, videos, isOwnProfile, isFollowing, followBusy, onFollow, onBack, onOpenVideo }) {
    ...
    <div style={{ height: "100%", background: T.bg, position: "relative", zIndex: 30, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ overflowY: "auto", flex: 1 }}>
        <div style={{ padding: "24px 20px 16px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <div style={{ width: 76, height: 76, borderRadius: "50%", background: T.lime, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700, color: T.bg, fontFamily: FONT }}>
          ...
          <div style={{ display: "flex", gap: 26, marginTop: 4 }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: T.paper, fontWeight: 700, fontFamily: FONT, fontSize: 15 }}>{abbreviate(owner.following_count || 0)}</div>
              <div style={{ color: T.paperDim, fontSize: 11.5, fontFamily: FONT }}>Following</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: T.paper, fontWeight: 700, fontFamily: FONT, fontSize: 15 }}>{abbreviate(owner.followers_count || 0)}</div>
              <div style={{ color: T.paperDim, fontSize: 11.5, fontFamily: FONT }}>Followers</div>
            </div>
            <div style={{ textAlign: "center" }}>
              <div style={{ color: T.paper, fontWeight: 700, fontFamily: FONT, fontSize: 15 }}>{abbreviate(owner.likes_count || 0)}</div>
              <div style={{ color: T.paperDim, fontSize: 11.5, fontFamily: FONT }}>Likes</div>
            </div>
          </div>
          {!isOwnProfile && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, padding: "0 16px 16px" }}>
          {theirVideos.length === 0 && (
            <div style={{ gridColumn: "1 / -1", textAlign: "center", color: T.paperDim, fontFamily: FONT, fontSize: 13, padding: "20px 0" }}>
              No reels from @{owner.username} in the current feed yet
            </div>
          )}
          {theirVideos.map((v) => (
            <button key={v.id} onClick={() => onOpenVideo(v)} style={{ aspectRatio: "9/13", borderRadius: 8, background: "linear-gradient(160deg,#2d3b1f,#141310 70%)", border: "none", padding: 0, cursor: "pointer", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", left: 6, bottom: 6, right: 6, color: T.paper, fontSize: 11, fontFamily: FONT, textAlign: "left" }}>
                {v.caption?.slice(0, 30)}
              </div>
            </button>
          ))}
        </div>
...
function FeedScreen({ api, myUserId, onOpenProfile }) {
  ...
  const toggleLike = async (video) => {
    ...
    try {
      await (wasLiked ? api.unlike(video.id) : api.like(video.id));
    } catch (e) {
      setVideos((vs) => vs.map((v) => v.id === video.id ? { ...v, is_liked: wasLiked, likes_count: video.likes_count } : v));
    } finally {
      setLikeBusy((s) => ({ ...s, [video.id]: false }));
    }
  };
  ...
  const toggleFollow = async (video) => {
    ...
    try {
      await (wasFollowing ? api.unfollow(username) : api.follow(username));
    } catch (e) {
      setVideos((vs) => vs.map((v) => v.owner.username === username ? { ...v, owner: { ...v.owner, is_following: wasFollowing } } : v));
    } finally {
      setFollowBusy((s) => ({ ...s, [username]: false }));
    }
  };
  ...
  const shareVideo = async (video) => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "GrowTok", text: `Check out @${video.owner.username} on GrowTok`, url });
      } else {
  ...
  const bumpCommentCount = (videoId, delta) => {
    setVideos((vs) => vs.map((v) => v.id === videoId ? { ...v, comments_count: v.comments_count + delta } : v));
  };
  ...
  return (
    <div style={{ position: "relative", height: "100%" }}>
      ...
      {shareToast && (
        <div style={{ position: "absolute", top: 56, left: "50%", transform: "translateX(-50%)", background: T.card, border: `1px solid ${T.line}`, borderRadius: 14, padding: "6px 14px", color: T.paper, fontSize: 12.5, fontFamily: FONT, zIndex: 6 }}>
          {shareToast}
        </div>
      )}
      {commentsFor && (
        <CommentsDrawer video={commentsFor} api={api} myUserId={myUserId} onClose={() => setCommentsFor(null)} onCountChange={(delta) => bumpCommentCount(commentsFor.id, delta)} />
      )}
    </div>
  );
}
...
function UploadScreen({ api, onUploaded }) {
  ...
  const onUpload = async () => {
    ...
    try {
      ...
      setStatus("done"); setFiles([]); setCaption("");
      onUploaded?.();
    } catch (e) {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 28px 70px", boxSizing: "border-box", textAlign: "center" }}>
      ...
      {files.length === 0 ? (
        ...
        <>
          <input placeholder="Caption for this batch (optional)" value={caption} onChange={(e) => setCaption(e.target.value)} style={{ width: "100%", boxSizing: "border-box", background: T.card, border: `1px solid ${T.line}`, borderRadius: 12, padding: "10px 14px", color: T.paper, fontFamily: FONT, fontSize: 13.5, outline: "none", marginBottom: 12 }} />
...
// ---------------------------------------------------------------------------
function ProfileScreen({ me, onLogout }) {
  if (!me) return null;
  const stats = [
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: "28px 20px 70px", boxSizing: "border-box" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
        ...
        <button onClick={onLogout} style={{ marginTop: 10, background: "none", border: `1px solid ${T.line}`, color: T.paper, borderRadius: 18, padding: "8px 22px", fontFamily: FONT, fontWeight: 600, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
...
// (caption text or @username), plus a trending-tags shortcut row.
// ---------------------------------------------------------------------------
function DiscoverScreen({ api, onOpenProfile }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await api.getFeed(0, 50);
        if (!cancelled) { setVideos(data || []); setStatus("ready"); }
      } catch (e) {
  ...
  const results = q
    ? videos.filter((v) =>
        v.caption?.toLowerCase().includes(q) || v.owner?.username?.toLowerCase().includes(q)
      )
    : videos;
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "18px 16px 70px", boxSizing: "border-box" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.card, border: `1px solid ${T.line}`, borderRadius: 20, padding: "10px 14px", marginBottom: 18 }}>
        ...
        <input
          ...
          placeholder="Search captions or @username"
          style={{ flex: 1, background: "none", border: "none", outline: "none", color: T.paper, fontFamily: FONT, fontSize: 14 }}
        />
        {query && (
          <button onClick={() => setQuery("")} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}><X size={15} color={T.paperDim} /></button>
        )}
      </div>
...
export default function GrowTokApp() {
  ...
  const [viewingProfile, setViewingProfile] = useState(null); // { owner, videos }
  ...
  if (!tokens) {
    return (
      <Shell>
        <ConnectScreen onAuthed={setTokens} />
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      <div style={{ position: "absolute", inset: 0 }}>
        ...
        {tab === "profile" && <ProfileScreen me={tokens.user} onLogout={() => setTokens(null)} />}
      </div>
          ...
          <CreatorProfileScreen
            ...
            onFollow={async () => {
              try {
                ...
                setViewingProfile((p) => ({
                  ...p,
                  videos: p.videos.map((v) => v.owner.username === p.owner.username ? { ...v, owner: { ...v.owner, is_following: !profileFollowing } } : v),
                }));
              } catch (_) {}
            }}
            onBack={closeProfile}
